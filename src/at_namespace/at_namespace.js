...
var at = ( function ( global ) {
	...
	,	_at = {
		...
		,	root : {}   //{Object} Pointer to root of current namespace (the global)
		}
	,	_lastSet
	,	_names = []
	;
	...
	function _getNs( obj ) {
		...
		for( var i = 0, len=_spaces.length; i < len; i++ ) {
			...
			if( _names.join("|").indexOf( _name ) === -1 ) {
				_names.push( _name );
			}
			if(_lastns){
	...
	,	obj            // {Object} Optional to initialize namespace.  Throws error if namespace already initialized
		...
		/**
		 * If prepended with "/" namespace resets namespace and starts over.
		 * For example:
		 *   at.set( "some.path" ); //adds some, and then path spaces onto existing space (if any)
		 *   at.set( "/foo.bar" );  //resets namespaces to "foo" then "foo.bar"
		 */
		if( _names[0].match( /^\// ) ){
			_spaces = [];
		...
		for(i = 0; i < _len; i++){
			...
			// #ifdef debug
			debug.info("@namespace: ", _spaces.slice().join("."));
			// #endif
	...
	_at.unset = function (atNamespace){
		...
		,	_names = ( typeof( _atSpace ) === "string" ) ? _atSpace.split(".").reverse() : [ _spaces[ _spaces.length - 1 ] ]
